...
\textbf{Result:} A direct comparison against the original 3-kernel pipeline is not feasible because the upstream attention dispatch is incompatible with our GQA head expansion.  We instead compare against PyTorch SDPA by routing all attention through it: with KV-cached decoding (seq\_q=1), SDPA is $\sim$9\,ms faster ($\sim$212 vs.\ $\sim$221\,ms) due to cuDNN's optimized single-row backend. This is why we use SDPA fallback for \texttt{seq\_q}$\leq$4 (Table~\ref{tab:ablation}: +3.4\,ms when disabled).  Flash attention's advantage is in the encoder and prefill, where large sequences ($\sim$750 tokens) would otherwise materialize the full score matrix.
