...
\latestchgblock{We later reintroduced the original 3-kernel path behind \texttt{GLM\_ASR\_ATTENTION\_MODE=three\_kernel} and benchmarked it against the current deployed path (\texttt{auto}: flash kernel for larger \texttt{seq\_q}, SDPA fallback for \texttt{seq\_q}$\leq$4) on the same H200 codebase.  In the warmup-corrected detailed benchmark, the current path reduces isolated component time from 651.51\,ms to 624.98\,ms (1.04$\times$), with lower prefill (13.37$\rightarrow$12.86\,ms) and 50-step decode cost (601.65$\rightarrow$575.64\,ms).  In the paired end-to-end student benchmark from the same job, however, the current path measured 219.8\,ms while the 3-kernel path measured 207.0\,ms.  We therefore treat this experiment as reproducible operator-level evidence rather than as a single universal end-to-end speedup result.  The clean end-to-end justification for keeping the SDPA fallback remains Table~\ref{tab:ablation}: disabling it costs +3.4\,ms.}

\latestsource{Source: \texttt{benchmarks/benchmarks\_attention.md}; script: \texttt{hw1-asr/flash\_vs\_three\_kernel\_job.sh}; raw logs: \texttt{logs/h200\_attention\_2237998/}.}
...
Section~5.3 & \texttt{benchmarks\_attention.md} & \texttt{flash\_vs\_three\_kernel\_job.sh} & \texttt{logs/h200\_attention\_2237998/...} \\
