...
\textbf{Result:} On the H200, we compared our flash attention kernel against PyTorch's built-in SDPA by routing all attention through SDPA.  With KV-cached decoding (seq\_q=1), SDPA is $\sim$9\,ms faster ($\sim$212\,ms vs.\ $\sim$221\,ms) because PyTorch's cuDNN attention backend is highly optimized for single-row queries.  This is why we use the SDPA fallback for \texttt{seq\_q}$\leq$4 (Table~\ref{tab:ablation}: +3.4\,ms when disabled).  Flash attention's advantage lies in the encoder and prefill phases, where large sequences ($\sim$750 tokens) would otherwise require materializing the full score matrix.
