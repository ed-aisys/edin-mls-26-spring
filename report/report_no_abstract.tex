...
The H200 ablation shows MLP fusion contributes only $-$0.7\,ms (Table~\ref{tab:ablation}), because cuBLAS already saturates the H200's compute throughput for these matrix sizes.
...
\textbf{Result:} Flash attention contributed significantly to the 209.8\,ms $\to$ 136.4\,ms improvement (combined with bf16 weights).  The SDPA fallback for decode saved an additional 3.5\,ms; the ablation confirmed +3.4\,ms cost when disabled (Table~\ref{tab:ablation}).
...
The H200 ablation shows this is the most impactful single optimization: +28.9\,ms when disabled (Table~\ref{tab:ablation}).
...
The ablation suggests the SDPA fallback threshold merits per-architecture tuning (\texttt{seq\_q}$\leq$1 was $-$1.4\,ms better than the current threshold of 4 on H200).

\textbf{Groupwork.} Ankush Burman: kernel implementation, optimization, benchmarking, profiling, report writing. Yash Chowdhary: kernel implementation, optimization, repo setup, branch management, cluster benchmarks. Meave: kernel implementation, optimization, report review, testing with longer audio files. Majed: kernel implementation, optimization, nsys profiling, report review.
