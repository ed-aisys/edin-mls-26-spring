...
\label{tab:benchmap}
\footnotesize
\begin{tabularx}{\linewidth}{@{}p{0.18\linewidth}p{0.16\linewidth}YY@{}}
\toprule
\textbf{Report element} & \textbf{Canonical doc} & \textbf{Script / runner} & \textbf{Artifact} \\
\midrule
\texttt{tab:e2e}, \texttt{tab:comparison} & \texttt{benchmarks.md} & \texttt{benchmark\_student.py} & H200 e2e bundle \texttt{h200\_e2e\_2225992} \\
\texttt{tab:detailed}, \texttt{tab:peroperator} & \texttt{benchmarks\_detailed.md} & \texttt{benchmark\_detailed.py} and \texttt{benchmark\_detailed\_job.sh} & H200 detailed bundle \texttt{h200\_detailed\_2236079} \\
\texttt{tab:ablation} & \texttt{benchmarks\_ablation.md} & \texttt{ablation\_test.py} and \texttt{ablation\_job.sh} & Generated files \texttt{ablation\_results.json} and \texttt{ablation\_results.md} \\
\texttt{tab:crossgpu\_main} (H200) & \texttt{benchmarks.md} & \texttt{benchmark\_student.py} & Same H200 e2e bundle \texttt{h200\_e2e\_2225992} \\
\texttt{tab:crossgpu\_main} (RTX~5090) & \texttt{benchmarks\_5090.md} & \texttt{benchmark\_student.py} & Embedded confirmation transcript in the benchmark doc \\
Section~5.3 & \texttt{benchmarks\_attention.md} & \texttt{flash\_vs\_three\_kernel\_job.sh} & H200 attention bundle \texttt{h200\_attention\_2238022} \\
\texttt{tab:progression}, \texttt{tab:rejected} & \texttt{benchmarks\_history.md} & Historical development runs & Extracted history at branch base \texttt{f8c2f36} \\
\bottomrule
\end{tabularx}
\end{table}
