...
\latestchgblock{\textbf{Latest update:} Section~5.3 now uses canonical rerun \texttt{2238022}, and \texttt{auto} is defined explicitly as the deployed mixed dispatch (flash for larger queries, SDPA fallback for tiny KV-cached decode steps), not as flash-only attention.}

\latestchgblock{We later reintroduced the original 3-kernel path behind \texttt{GLM\_ASR\_ATTENTION\_MODE=three\_kernel} and benchmarked it against the current deployed path.  Here, \texttt{auto} does \emph{not} mean ``flash everywhere'': it is the real deployed mixed dispatch, using the Triton flash kernel for larger \texttt{seq\_q} and PyTorch SDPA only for tiny KV-cached decode steps (\texttt{seq\_q}$\leq$4).  In the updated same-codebase rerun, the student benchmark now also discards one full cold benchmark pass and averages three measured passes.  Under that methodology, the current path is slightly faster end-to-end (210.9\,ms vs.\ 212.0\,ms) while preserving 100\% accuracy.  The warmup-corrected detailed benchmark tells the same story: isolated component time falls from 641.09\,ms to 625.04\,ms (1.03$\times$), with lower prefill (13.36$\rightarrow$12.82\,ms) and 50-step decode cost (591.41$\rightarrow$575.89\,ms).  We therefore treat FlashAttention-style dispatch as a modest but consistent same-codebase improvement rather than a dramatic standalone headline result.  The clean end-to-end justification for keeping the SDPA fallback remains Table~\ref{tab:ablation}: disabling it costs +3.4\,ms.}

\latestsource{Source: \texttt{benchmarks/benchmarks\_attention.md}; script: \texttt{hw1-asr/flash\_vs\_three\_kernel\_job.sh}; raw logs: \texttt{logs/h200\_attention\_2238022/}.}
...
\latestchgblock{Table~\ref{tab:benchmap} provides a direct trace from the main benchmark-backed report claims to their canonical benchmark documents, scripts, and raw artifacts.  The intended lookup path is: report table $\rightarrow$ canonical benchmark doc $\rightarrow$ script/runner $\rightarrow$ raw artifact or generated result file.}

\begin{table}[h]
\centering
\caption{\latestchglabel\ Benchmark source map for the main report claims.}
\label{tab:benchmap}
\small
\begin{tabular}{@{}p{0.18\linewidth}p{0.18\linewidth}p{0.24\linewidth}p{0.26\linewidth}@{}}
\toprule
\textbf{Report element} & \textbf{Canonical doc} & \textbf{Script / runner} & \textbf{Artifact} \\
\midrule
\texttt{tab:e2e}, \texttt{tab:comparison} & \texttt{benchmarks.md} & \texttt{benchmark\_student.py} & \texttt{logs/h200\_e2e\_2225992/...} \\
\texttt{tab:detailed}, \texttt{tab:peroperator} & \texttt{benchmarks\_detailed.md} & \texttt{benchmark\_detailed.py} / \texttt{benchmark\_detailed\_job.sh} & \texttt{logs/h200\_detailed\_2236079/...} \\
\texttt{tab:ablation} & \texttt{benchmarks\_ablation.md} & \texttt{ablation\_test.py} / \texttt{ablation\_job.sh} & \texttt{hw1-asr/ablation\_results.\{json,md\}} \\
\texttt{tab:crossgpu\_main} (H200) & \texttt{benchmarks.md} & \texttt{benchmark\_student.py} & \texttt{logs/h200\_e2e\_2225992/...} \\
\texttt{tab:crossgpu\_main} (RTX~5090) & \texttt{benchmarks\_5090.md} & \texttt{benchmark\_student.py} & embedded transcript \\
Section~5.3 & \texttt{benchmarks\_attention.md} & \texttt{flash\_vs\_three\_kernel\_job.sh} & \texttt{logs/h200\_attention\_2238022/...} \\
\texttt{tab:progression}, \texttt{tab:rejected} & \texttt{benchmarks\_history.md} & historical development runs & extracted history at \texttt{f8c2f36} \\
\bottomrule
\end{tabular}
\end{table}
...
\label{tab:codemap}
\small
\begin{tabular}{@{}p{0.17\linewidth}p{0.31\linewidth}p{0.30\linewidth}p{0.14\linewidth}@{}}
\toprule
...
Optimized template runtime & Selects GPU-specific tile/configuration choices, wires the fused Triton kernels into the Hugging Face model, and implements KV-cached generation. & \texttt{template/layers.py}: \texttt{GPUProfile}, \texttt{\_compute\_attention\_tiles}, \texttt{\_generate\_v8b}, \texttt{\_try\_patch\_v8b} & Secs.~3--5, conclusion \\
Attention subsystem & Implements the main attention dispatch: FlashAttention for long sequences, SDPA fallback for tiny decode steps, and the reintroduced 3-kernel path for controlled comparison. & \texttt{template/attention.py}: \texttt{flash\_attention\_kernel}, \texttt{scaled\_dot\_product\_attention} & Sec.~5.3, Table~\ref{tab:peroperator} \\
RoPE subsystem & Applies rotary position embeddings with a fused kernel path and the runtime wrapper used by the decoder. & \texttt{template/rope.py}: \texttt{fused\_rope\_pair\_kernel}, \texttt{apply\_rotary\_pos\_emb} & Kernel-fusion discussion, appendix \\
Baseline reference implementation & Provides the comparison target for end-to-end and per-operator speedups against our optimized template. & \texttt{example/layers.py}, \texttt{example/attention.py}, \texttt{example/rope.py} & Tables~\ref{tab:comparison}, \ref{tab:peroperator}, \ref{tab:crossgpu_main} \\
Evidence bundle & Stores the canonical benchmark writeups and the pulled raw H200 artifacts used to trace report numbers back to scripts and runs. & \texttt{benchmarks/*.md}, \texttt{logs/h200\_*} & Appendix A.2--A.3 \\
\bottomrule
\end{tabular}
\end{table}

\latestsource{Structure based on two documentation patterns: the C4 model's abstraction-first ``maps of your code'' and arc42's level-1 building-block/source-mapping guidance.  In practice, that means naming a small set of coherent building blocks, stating each block's responsibility, and pointing to the 2--3 most important source locations rather than dumping a flat symbol list.}
