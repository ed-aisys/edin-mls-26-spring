...
\begin{abstract}
We design, implement, and benchmark custom Triton GPU kernels for the GLM-ASR automatic speech recognition pipeline, achieving a 2.27$\times$ speedup over the provided baseline on the Edinburgh teaching cluster's H200 GPU.
...
We test each component individually and integrate them to ensure correctness at each stage.
...
We validated these choices through a 22-configuration ablation study on the H200 (Appendix Table~\ref{tab:ablation}), which confirmed that \texttt{num\_stages}=2 and \texttt{num\_warps}=8 are each worth $\sim$12\,ms.
...
The stock \texttt{generate()} reprocesses the full growing sequence on every decode step ($O(n^2)$).  We implemented a KV-cached generation loop that processes only the new token each step, appending its K/V to a cache.  This is monkey-patched onto the model via \texttt{\_try\_patch\_v8b()} (\texttt{layers.py:1482}), called during \texttt{Linear.\_\_init\_\_}.  Impact: \textbf{$-$7.6\,ms} (121.1 $\to$ 113.5\,ms).

\textbf{Fused Q+K RoPE kernel.}
Rotary position embeddings~\citep{su2024roformer} require applying the same rotation to both Q and K.  The baseline uses two separate kernel launches; we fuse them into \texttt{fused\_rope\_pair\_kernel} (\texttt{rope.py:189}), halving launch overhead.  Impact: \textbf{$-$11.8\,ms} (136.4 $\to$ 124.6\,ms).
...
On the H200 teaching cluster, our combined optimizations reduced inference from 464.1\,ms to 204.8\,ms (2.27$\times$).  Ablation testing (Appendix Table~\ref{tab:ablation}) confirmed that fused RoPE (+28.9\,ms), double-buffering (+12.2\,ms), and warp count (+12.0\,ms) are the three most impactful parameters.
...
(4)~Problems arise when kernels are composed in the full pipeline, as cache interactions between adjacent kernels produce behaviors not visible in isolated benchmarks.
...
\textbf{Future work.}
We would investigate \texttt{torch.compile}, speculative decoding, and INT8/FP8 quantization.
\textbf{Groupwork} (all contributed kernel implementation and optimization). Ankush: benchmarking, profiling, report. Yash: repo, cluster benchmarks. Meave: report review, audio testing. Majed: nsys profiling, report review.
...
\subsection{H200 Ablation Study}

\begin{table}[h]
\centering
\caption{H200 ablation: top-impact parameter changes (baseline: 205.2\,ms, 22 configs tested, all 100\% accuracy).}
\label{tab:ablation}
\small
\begin{tabular}{@{}lrrl@{}}
\toprule
