...
The fp16-throughout pipeline was the single largest contributor ($-$11.5\,ms on RTX~5090), followed by fused RoPE ($-$11.8\,ms), Flash Attention (part of $-$73.4\,ms combined with bf16 weights), and KV caching ($-$7.6\,ms).  Critically, removing \texttt{.float()} casts---a one-line change repeated across $\sim$120 call sites---yielded $-$7.5\,ms alone, illustrating that the highest-impact optimizations are often not algorithmic but rather eliminating unnecessary data movement.
...
(2)~Attention is the most challenging kernel to optimize because the tile size, shared memory capacity, warp count, and pipeline stage count are tightly coupled; a configuration that works on the H200's 228\,KB shared memory crashes on the RTX~5090's 99\,KB.
(3)~Profile before optimizing: our initial assumption that compute-bound matmul was the bottleneck was wrong---decoder decode steps (bandwidth-bound attention + KV reads) dominated baseline time.
(4)~Testing each functional block independently was simple enough to do; however, problems arose when functional blocks were connected to each other, as data layout assumptions and cache interactions between adjacent kernels produced behaviors not visible in isolated benchmarks.

\textbf{Surprising findings.}
The autotune system found \emph{worse} configurations than hand-tuning (101.6\,ms vs.\ 98.5\,ms), because micro-benchmarks miss inter-kernel cache effects.  Grid swizzling---a well-known L2 optimization---regressed by +18\,ms on the RTX~5090, likely because its large 98\,MB L2 already provides excellent locality.

\textbf{Future work.}
We would investigate \texttt{torch.compile} for automatic fusion, speculative decoding, and INT8/FP8 quantization.

\textbf{Groupwork.}
All members: kernel implementation and optimization.  Ankush: benchmarking, profiling, report.  Yash: repo setup, branch management, cluster benchmarks, review.  Majed: nsys profiling.
